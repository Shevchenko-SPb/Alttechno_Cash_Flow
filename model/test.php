...
function getDataArray($datePeriod)
{
    $arCashIn = array(
        "DEALS" => [
            "DEAL_ID_407" => [],
    ...
    #ElList_ID
    define("SQL_GET_DATA_BY_DATE", "SELECT biep.IBLOCK_ELEMENT_ID 
                                 ...
                                 AND `IBLOCK_PROPERTY_ID` = 405 AND `VALUE` BETWEEN '%s' AND '%s') ORDER BY `IBLOCK_ELEMENT_ID` ASC;");

    $sql1 = sprintf(SQL_GET_DATA_BY_DATE, $dateFrom, $dateTo);
    $sql = $connection->Query($sql1);
    ...
    return json_encode($arCashIn);
//        return $arCashIn;
}

$datePeriod = json_decode(file_get_contents('php://input'),true);
echo getDataArray($datePeriod);
die();
